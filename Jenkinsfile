    ...
    stages {
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=ub-application \
                    -Dsonar.projectKey=ub-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Building"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                            sh "docker build -t uber-application ." 
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag uber-application yash5090/uber-application:latest " 
                        //}
                    }
                }
            }
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/uber-application:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/uber-application:latest'
                       sh 'docker-scout cves yash5090/uber-application:latest'
                       sh 'docker-scout recommendations yash5090/uber-application:latest'
                       sh 'docker-scout attestation yash5090/uber-application:latest'
                   }
                }   
            }
        }
